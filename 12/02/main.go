...
func visited(path []string, name string) bool {
	// don't re-visit start.
	if name == "start" {
		return true
	}

	// check if any small cave has been traversed twice already.
	var twice bool
	var count = make(map[string]bool)
	for _, s := range path {
		if _, ok := count[s]; ok && strings.ToUpper(s) != s {
			twice = true
			break
		}
		count[s] = true
	...
	// check if this cave has been traversed already.
	for _, s := range path {
		if s == name && twice {
			return true
		}
